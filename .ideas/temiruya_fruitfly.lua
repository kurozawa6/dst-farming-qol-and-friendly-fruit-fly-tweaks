...
local function uninumToString(num)
    local str = "uni_num_"..(num and num or "(nil)")
    return str
end
...
--    if not TheSim:FindFirstEntityWithTag("friendlyfruitfly") then
    if (1==1) then --changed for lua extension
...
local function friendlyfn()
    ...
    local function OnPreLoad(inst, data)
        if data and data.uni_num then
            inst.uni_num = data.uni_num
        end
        inst:AddTag(uninumToString(inst.uni_num))
    end

    local function OnSave(inst, data)
        data.uni_num = inst.uni_num
    end
    ...
    inst.components.sanityaura.aura = TUNING.SANITYAURA_TINY
--[[ -- for debug
    function inst.components.inspectable.descriptionfn(inst, viewer)
        local str = inst.prefab.."\n".."uni_num is "..(inst.uni_num and inst.uni_num or "nil")
        return str
    end
]]    
...
local function OnSave(inst, data)
    ...
    data.uni_num = inst.uni_num and inst.uni_num or GetTime()
end

--local function SpawnFriendlyFruitFly(inst)
local function SpawnFriendlyFruitFly(inst, uni_num)
    local x, y, z = inst.Transform:GetWorldPosition()
    ...
    if fruitfly ~= nil then
        ...
        fruitfly.uni_num = uni_num
        fruitfly:AddTag(uninumToString(uni_num))
        return fruitfly
    end
end
...
--        local fruitfly = TheSim:FindFirstEntityWithTag("friendlyfruitfly") or SpawnFriendlyFruitFly(inst)
        local fruitfly = TheSim:FindFirstEntityWithTag(uninumToString(inst.uni_num)) or SpawnFriendlyFruitFly(inst, inst.uni_num)
        if fruitfly ~= nil and
...
return
	Prefab("lordfruitfly", fn, assets, prefabs),
    Prefab("friendlyfruitfly", friendlyfn, friendlyassets),
    Prefab("fruitflyfruit", fruitfn, fruitassets, fruitprefabs)